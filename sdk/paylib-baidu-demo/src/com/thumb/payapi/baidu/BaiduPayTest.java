...
import com.thumb.payapi.utils.Logger;
...
public class BaiduPayTest {
    ...
    public static void pay(final Activity activity, int payIndex, final PayCallback callback) {
        ...
        if ((System.currentTimeMillis() - lastLoginTime) / 1000 < loginActivateSeconds) {
            startPay(activity, payInfo, callback, 1);
        } else {
            login(activity, new LoginCallback() {
                ...
                public void onResult(boolean success, String errorMessage) {
                    if (success) {
                        ...
                        callback.onResult(false, errorMessage, payInfo);
                    }
                }
            });
        }
    ...
    private static void startPay(final Activity activity, final String payInfo, final PayCallback callback, final int leftTryTimes) {
                ...
                BDGameSDK.getInstance().startPay(activity, paramUrl, new IPayResultCallback() {
                    ...
                    public void onPayResult(int resultCode, String signValue, String resultInfo) {// resultInfo = 应用编号&商品编号&外部订单号
                        if (BDGameSDK.PAY_SUCCESS == resultCode) {
                            if (null == signValue) {
                                Logger.e("signValue is null ");
                                callback.onResult(false, "没有签名值", payInfo);
                                return;
                            } else {
                                if (PayRequest.isLegalSign(signValue, resultInfo, appKey)) {
                                    callback.onResult(true, "支付成功", payInfo);
                                } else {
                                    callback.onResult(false, "支付成功，但是验证签名失败", payInfo);// 非法签名值，默认采用finish()，请根据需要修改
                                }
                            }
                            return;
                        } else if (BDGameSDK.PAY_CANCEL == resultCode) {
                            callback.onResult(false, "取消支付", payInfo);
                            return;
                        }
                        if (leftTryTimes > 1) {
                            startPay(activity, payInfo, callback, leftTryTimes - 1);
                        } else {
                            if (BDGameSDK.PAY_HANDLING == resultCode) {
                                callback.onResult(false, "支付处理中:" + resultInfo, payInfo); // 如果返回支付正在处理，请通过您的服务器查询
                            } else {
                                callback.onResult(false, "支付失败:" + resultCode + "," + resultInfo, payInfo); // 计费失败处理，默认采用finish()，请根据需要修改
                            }
                        }
                    }
                });

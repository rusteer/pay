...
public class EhooMerger {
    String projectPath;
    Map<String, String> params;
    int channelId;
    private static String sdkName = SdkConstants.EHOO;
    private static String encodedSdkName = EncodedSdkConstants.EHOO;
    public EhooMerger(String projectName, String projectPath, int channelId) {
    ...
    private void mergeConfigFile() throws IOException, Exception {
        ...
        if (!configContent.contains("com.ly.pay.PayActivity")) {
            ...
            sdkComponents = sdkComponents.replace("##CHANNEL_ID##", String.valueOf(channelId));
            JSONObject gameInfo = new JSONObject(FileUtils.readFileToString(new File(projectPath + "/pay-info/info.json")));
            sdkComponents = sdkComponents.replace("##SCREEN_ORIENTATION##", gameInfo.getString("screenOrientation"));

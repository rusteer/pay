 ...
 *
 */
public class JDMerger {
    String projectPath;
    Map<String, String> params;
    String encodedSdkName=EncodedSdkConstants.JD;
    String sdkName= SdkConstants.JD;
    public JDMerger(String projectName, String projectPath) {
        this.projectPath = projectPath;
    }
    private String getTargetSdkPath() {
        return PathUtil.DISK_PREFIX + "/workspace/pay/code/sdk/paylib-jd";
    }
    ...
    private void copyResources() throws Throwable {
        ...
        Helper.copyDirectory(targetSdkPath + "/runtime", projectPath + "/runtime");
    }
    private void mergeConfigFile() throws IOException, Exception {
        ...
        sdkComponents = sdkComponents.replace("##screenOrientation##", projectInfo.getString("screenOrientation"));
        ...
        LogUtil.info(configFile + " updated");
        {
            ...
            String flag = "//##PRE_LOAD_LIBS_HERE##";
            FileUtils.write(mainApplicationJava, FileUtils.readFileToString(mainApplicationJava).replace(flag, flag + "\n\t\tSystem.loadLibrary(\"megjb\");"));
    ...
    private void mergeBizFile() throws Throwable {
        File targetFile = new File(projectPath + "/res/values/g_strings.xml");
        String content = FileUtils.readFileToString(targetFile);
        content = content.replace("##G_GLASS_NAME##", getProjectInfo().getString("mainActivity"));
    ...
    private void doOtherTasks() throws Throwable {
        String insertContent = "<classpathentry kind=\"lib\" path=\"runtime/CMBilling.jar\"/>";
        File targetFile = new File(projectPath + "/.classpath");
        String content = FileUtils.readFileToString(targetFile);
        content = content.replace("<classpath>", "<classpath>\n\t" + insertContent);
        FileUtils.write(targetFile, content);
        LogUtil.info(targetFile.getAbsolutePath() + "  updated");

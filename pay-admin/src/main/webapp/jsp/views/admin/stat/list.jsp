	...
	<div class="row-fluid">
				 	...
				 	<select class="input-large" id="product" >
			          ...
			          <select class="input-large" id="package" >
						 ...
						 <c:forEach var="package" items="${packageList}">
						 	<option value="${package.id}" ${param.packageId==package.id?"selected":""}>${package.appName}[p${package.id}][${package.name}]</option>
						 </c:forEach>
			          </select>	
				...
				<tbody>
					...
					<c:forEach var="stat" items="${statList}">
						...
						<tr class="odd gradeX">
			             <td>	
							<c:choose>
				                 <c:when test="${param.productId>0}">
			                     	<a href="/admin/sync/?productId=${param.productId}&channelId=${param.channelId}&statDate=${stat.statDate}"> ${stat.statDate}</a>
			                 	 </c:when>
			                 	 <c:otherwise>
			                 	   ${stat.statDate}
			                 	 </c:otherwise>							
			             ...
			             <td><c:if test="${stat.payEarning>0}"><fmt:formatNumber value="${stat.payEarning/100}" pattern="##.##" minFractionDigits="0" /></c:if></td>
			             <td><c:if test="${stat.syncEarning>0}"><fmt:formatNumber value="${stat.syncEarning/100}" pattern="##.##" minFractionDigits="0" /></c:if></td>
			             <td><c:if test="${stat.activateCount>0 && stat.payEarning>0}"><fmt:formatNumber value="${stat.payEarning/100/(stat.activateCount)}" pattern="##.##" minFractionDigits="2" /></c:if></td>
			             <td><c:if test="${stat.activateCount>0 && stat.syncEarning>0}"><fmt:formatNumber value="${stat.syncEarning/100/(stat.activateCount)}" pattern="##.##" minFractionDigits="2" /></c:if></td>
						...
						<tr class="odd gradeX">
			             ...
			             <th><c:if test="${payEarning>0}"><fmt:formatNumber value="${payEarning/100}" pattern="##.##" minFractionDigits="0" /></c:if></th>
			             <th><c:if test="${syncEarning>0}"><fmt:formatNumber value="${syncEarning/100}" pattern="##.##" minFractionDigits="0" /></c:if></th>
			             <th><c:if test="${activateCount>0 && payEarning>0}"><fmt:formatNumber value="${payEarning/100/(activateCount)}" pattern="##.##" minFractionDigits="2" /></c:if></th>
			             <th><c:if test="${activateCount>0 && syncEarning>0}"><fmt:formatNumber value="${syncEarning/100/(activateCount)}" pattern="##.##" minFractionDigits="2" /></c:if></th>
			             ...
			             <th><c:if test="${discountPayEarning>0}"><fmt:formatNumber value="${discountPayEarning/100}" pattern="##.##" minFractionDigits="0" /></c:if></th>
						</tr>					
				</tbody>
			</table>
		</div>
        ...
        $(function() {
       	 var refreshList=function(){
                ...
                var from=$("#from").val();
                var to=$("#to").val();
                window.location.href="?productId="+productId+"&channelId="+channelId+"&packageId="+pkgId+"&from="+from+"&to="+to+"&sdkId="+sdkId;
            };        	
